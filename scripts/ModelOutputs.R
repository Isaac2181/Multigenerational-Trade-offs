...
make_mixed_model_table <- function(model,
                                   title = "Mixed Model Summary",
                                   subtitle = NULL,
                                   anova_type = 2  # 2 for Type II, 3 for Type III
) {
  ...
  if (is_glmmTMB) {
    # Conditional model
    model_formula <- paste(
      deparse(formula(model, component = "cond")),
      collapse = " "
    )
    
    disp_formula <- tryCatch(
      paste(deparse(formula(model, component = "disp")), collapse = " "),
      error = function(e) NULL
    )
    zi_formula <- tryCatch(
      paste(deparse(formula(model, component = "zi")), collapse = " "),
      error = function(e) NULL
    )
  ...
  fx$p.value_fmt <- ifelse(
    fx$p.value < 0.001,
    "<0.001",
    sprintf("%.3f", fx$p.value)
  )
  ...
  if (is_lme4) {
    ...
    if (!inherits(rv, "try-error") && nrow(rv) > 0) {
      rand_rows_var <- data.frame(
        Term  = paste0(
          "τ00 ",
          rv$group,
          ifelse(rv$term == "(Intercept)", "", paste0(":", rv$term))
        ),
        Value = rv$estimate,
        stringsAsFactors = FALSE
      )
    ...
    # residual variance (for Gaussian LMMs)
    ...
    if (!inherits(vc_df, "try-error") && nrow(vc_df) > 0) {
      rand_rows_var <- data.frame(
        Term = paste0(
          "τ00 ",
          vc_df$grp,
          ifelse(vc_df$var1 == "(Intercept)", "", paste0(":", vc_df$var1))
        ),
        Value = vc_df$vcov,
        stringsAsFactors = FALSE
      )
    ...
    # group sizes from random terms
    rand_terms <- lme4::findbars(formula(model, component = "cond"))
    if (length(rand_terms) > 0) {
      grp_names <- vapply(rand_terms, function(x) as.character(x[[3]]), character(1))
      grp_names <- unique(grp_names)
      
      mf <- model$frame
      n_list <- lapply(grp_names, function(g) length(unique(mf[[g]])))
  ...
  ## --- 4. ANOVA block (Type II / III from car::Anova) ---------------
  ...
  ## --- 5. Format fixed/random for final table ----------------------
  ...
  if (!is.null(random_disp) && nrow(random_disp) > 0) {
    random_disp$Value <- ifelse(
      is.numeric(random_disp$Value),
      round(random_disp$Value, 2),
      random_disp$Value
    )
  } else {
    random_disp <- data.frame(
      Term  = "Observations",
      Value = stats::nobs(model),
      stringsAsFactors = FALSE
    )
...
################################################################################
# Helper: Survival model table (Cox-style, but same layout)                    #
################################################################################
...
make_survival_table <- function(model,
                                title = "Mixed Model Summary",
                                subtitle = NULL,
                                anova_type = 2) {
  stopifnot(requireNamespace("gt",          quietly = TRUE))
  ...
  # Reuse make_mixed_model_table logic:
  tbl <- make_mixed_model_table(
    model       = model,
    title       = title,
    subtitle    = subtitle,
...
# NOTE: your original script used F3_ASR; model is named F3_age in analysis
...
P0LRS <- make_mixed_model_table(P0_LRS, title = "P0 Lifetime Reproductive Success")
gt::gtsave(P0LRS, file = file.path(out_dir, "P0LRS.docx"))

F1LRS <- make_mixed_model_table(F1_LRS, title = "F1 Lifetime Reproductive Success")
gt::gtsave(F1LRS, file = file.path(out_dir, "F1LRS.docx"))

F3LRS <- make_mixed_model_table(F3_LRS, title = "F3 Lifetime Reproductive Success")
gt::gtsave(F3LRS, file = file.path(out_dir, "F3LRS.docx"))

# Rate-sensitive fitness
P0fit <- make_mixed_model_table(P0_fit, title = "P0 Rate Sensitive Fitness")
gt::gtsave(P0fit, file = file.path(out_dir, "P0fit.docx"))

# NOTE: your original script used F1_fit1; model is named F1_fit in analysis
F1fit <- make_mixed_model_table(F1_fit, title = "F1 Rate Sensitive Fitness")
gt::gtsave(F1fit, file = file.path(out_dir, "F1fit.docx"))

F3fit <- make_mixed_model_table(F3_fit, title = "F3 Rate Sensitive Fitness")
gt::gtsave(F3fit, file = file.path(out_dir, "F3fit.docx"))

# Survival models (if you are including survival in the repo/manuscript)
...
F3sur <- make_survival_table(F3_sur, title = "F3 Survival")
